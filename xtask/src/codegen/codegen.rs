...
fn codegen_base_addresses_struct(aobs: &[Box<dyn Aob>]) -> String {
    ...
    // Hand-rolled rather than derived: `#[derive(Debug)]` prints addresses in
    // decimal, which is useless when you want to check one against a disassembler.
    generated.push_str("impl std::fmt::Debug for BaseAddresses {\n");
    generated
        .push_str("    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {\n");
    generated.push_str("        f.debug_struct(\"BaseAddresses\")\n");
...
pub fn codegen_base_addresses(
    ...
    let mut version_data = patches_paths
        ...
        .filter_map(|exe| {
            ...
            let version =
                pe_file.resources().unwrap().version_info().unwrap().fixed().unwrap().dwFileVersion;
